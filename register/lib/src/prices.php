...
$description = "";
	...
	// replace the basic price with the financial aid co-pay
	...
	// subtract early bird discount
	if ( applyEarlyBirdDiscount($priceType) ) 
	{
		$myPrice -= $PDC_Prices["EarlyBirdDiscount"];
		if ($myPrice < 0 ) $myPrice = 0;		// Don't go below zero!
		$description = $description . "Earlybird discount applies<br />";
	}
...
function applyLateFee($personType)
{
	...
	// for 2012, disable the late fee entirely
	return FALSE;
...
function applyEarlyBirdDiscount($personType)
{
global $PDC_Prices, $PDC_Today, $PDC_RegistrationStartDate, $PDC_RegistrationDeadline;
	// for testing purposes, allow forcing of late fee in the querystring
	if ( $PDC_Today <= $PDC_RegistrationDeadline
		|| ( isset($_REQUEST['debugEarlyBirdDiscount']) && $_REQUEST['debugEarlyBirdDiscount'] ) )
	{

		if ( isset($_REQUEST['debugLateFee']) && $_REQUEST['debugLateFee'] ) 
			return FALSE;
			
		if ($personType != "Adult" && $personType != "Aide" ) return TRUE;
	}
	else
		return FALSE;
...
global $PDC_Prices, $PDC_Today, $PDC_RegistrationStartDate, $PDC_RegistrationDeadline;
	...
	if ( $PDC_Today > $PDC_RegistrationDeadline || (isset($_REQUEST['debugLateFee'])) )
		return FALSE;
