...
class PDC_Registration { 
  ...
  function initiate($params) {
    ...
    //$this->internal_id = $family->getRecordId();
    $_SESSION['recid'] = $family->getRecordId();
  }
  ...
  function new_camper( $params ) {
	...
	$new_record->setField("Special Requests", $params['specialRequests']);
	...
	$new_record->setField('hidden name', $params['hiddenName']);


	
	$new_record->setField('Price', $params['price']);

    // duplicated information from the family record
	...
	$new_record->setField('Child Notes',  $this->notesToRegistrar);
