...
require_once("models/volunteer_model.php");
...
{
	?>
  <!-- <p class="bolded-text color-red">Day Camp is running completely full this year with a wait list of over 200.  We are no longer accepting camper registrations without a Full Time Adult Unit Leader volunteer.</p> -->
  <h3>Register before <?php echo date("F j", $PDC_RegistrationDeadline); ?>!</h3>
...
if (($PDC_Today >= $PDC_RegistrationStartDate && $PDC_Today < $PDC_RegistrationDeadline )
	|| isset ($_REQUEST["debugAllowEarlyRegistration"] ) 
	|| isset ($_REQUEST["expressRegistration"] )
  || isset ($_REQUEST["postDeadlineRegistration"] )
	)
// echo $early_closure;
  if($early_closure == true){
    earlyClosure();
  }else{
...
?>
	<tr>
	<th colspan="2">Do you want to apply for Financial Aid?<br/>
	<span class="bolded-text">Financial Aid requests MUST include an Adult Volunteer</span>
	<br>A financial aid form will be sent to you; please complete and return as soon as possible.  A $125 co-pay is due when you register.</th>
	<td><select name="paymentFinancialAid" id="financialAid"><option>No</option><option>Yes</option></select></td>
	</tr>
...
function earlyClosure(){
  echo "<p style=\"font-style:bold; color: red;\">Day Camp is running completely full this year with a wait list of over 200.  We are no longer accepting camper registrations without a Full Time Adult Unit Leader volunteer.</p>";
}
...
<?php
	...
	{
	?>
		<tr><th colspan="3">Aides (Girls entering grades 10-12 or first year of college).<br/>Aides must be at camp for full days. They can attend only certain days but those days must be full days.
    	</th></tr>
	...
	<?php
		...
		$homeVolunteerSpots = $volunteer->getHomeVolunteerSpots();
	?>
    ...
    <tr><td>&nbsp;</td><td>Adult Volunteer Type<br/>
    	<?php 
    		echo($homeVolunteerSpots>=0 ? "" : "<span class='italic'>Note: We have enough volunteers to fill all our at-home jobs</span>");
    	?>
    	<td>
        <select name="volunteerType">
            <option value="VolunteerNone" selected="selected">No volunteer</option>
            <option value="VolunteerAtHome"<?php echo($homeVolunteerSpots>=0 ? "" : " disabled");?>>At-home volunteer (about 20 hours)<?php echo " - $homeVolunteerSpots left!"?></option>
