...
function FMStudio_PayPal_Notify_OLD($Connection,$Layout,$business,$Invoice,$Status,$Fields,$Values,$script,$sandbox,$rootPath) {
	$vars = array();
...
function FMStudio_PayPal_Notify($Connection,$Layout,$business,$Invoice,$Status,$Fields,$Values,$script,$sandbox,$rootPath) {
	...
	// Security checks come first
	if($vars['receiver_email'] != $business) {
		error_log('SECURITY ERROR: RECEIVER EMAILS DO NOT MATCH:' . $vars['receiver_email']);
		return '';
	}
	...
	if($vars['notify-validate'] != 'VERIFIED') {
		error_log('VALIDATION FAILED: Invoice: ' . $vars['invoice']);
		return '';
	}
	
	if(is_a($Connection,'FileMaker')) {	  
		$record = $Connection->getRecordById('cgi_web-family', $vars['invoice']);
		if(FileMaker::isError($record)) {
		   error_log("Problem Finding Record: Invoice: " . $vars['invoice'] . ": " . $record->getMessage());
		   return '';
		}
  	...
  	$result = $record->commit();
		...
		if(FileMaker::isError($result)) {
			$success = False;
		   error_log("FileMaker Error on Commit: Invoice: " . $vars['invoice']);
		   return '';
		}
		else{
		  error_log("GOOD: Invoice: " . $vars['invoice']);
		}
		
		$result = array(
			"contact_email" => $record->getField("contactEmail"),
			"invoice_num" => $vars['invoice'],
			"result" => $success
		);
		return $result;
	}
