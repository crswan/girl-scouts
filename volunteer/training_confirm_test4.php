...
function mail_body()
{
	...
	{
	if ($trainingClasses[i] == 'pdc1-1') {
		echo "Class Description: Orientation & Background - Required for all 1st Year Volunteers \n" .
		"Date: 4/1\n" .
		"Day: Tuesday\n" .
		"Time: 6:30-9:30ppm\n";
	}
	else if ($trainingClasses[i] == 'pdc1-2') {
		echo "Class Description: Orientation & Background - Required for all 1st Year Volunteers \n" .
		"Date: 4/9\n" .
		"Day: Wednesday\n" .
		"Time: 6:30-9:30pm\n";
...
//print_r($_POST);
	...
	*/
	...
	$esc_classDescription = mysql_real_escape_string ($_POST["description"]);
	$esc_date = mysql_real_escape_string ($_POST["date"]);
	$esc_day = mysql_real_escape_string ($_POST["day"]);
	$esc_time = mysql_real_escape_string ($_POST["time"]);
	$esc_place = mysql_real_escape_string ($_POST["place"]);
	...
	//Sharon's email when ready: srtorrano@yahoo.com
		$trainingClasses = $_POST["trainingClasses"];

echo "Can you see this: " . $esc_classDescription . " " . $esc_date . " " . $esc_day . " " . $esc_time . " " . $trainingClasses[$i];

	$to = $esc_email;
	$header = "Bcc: crswan@stanford.edu";
	$body = "Name: " .  $esc_firstName  . " " .  $esc_lastName .  
	" is signed up for the following training:\n\n " . 
	"Name: " . $esc_firstName . " " . $esc_lastName . "\n" . 
	"Email: " . $esc_email . "\n" .  
	"Phone: " . $esc_phone . "\n" . 
	"Years at Camp: " . $esc_yearsAtCamp . "\n" . 
	"Comments: " . $esc_comments . "\n" . 
	"Class Number: " . $trainingClasses[$i] . "\n" . mail_body($trainingClasses[$i]) . "\n" ;
	
	$body = wordwrap($body, 70);
	echo $body;
	//Send the email
	mail($to,'New Training Registration!',$body, $header);
